...
```{r echo = FALSE}
Bootstrap_app <- list(name = "hello", url = "goodbye")
Spread_app <- list(name = "hello", url = "goodbye")
Density_app <- list(name = "hello", url = "goodbye") 
Regression_app <- list(name = "hello", url = "goodbye")
Stratify_app <- list(name = "hello", url = "goodbye")
T_app <-    list(name = "hello", url = "goodbye")
```


Response     | Explanatory  | Statistical technique   | Little App
-------------|--------------|-------------------------|-------------
Quantitative | Quantitative |  linear regression      | [Little_App_Regression](`r Regression_app$url`)
Quantitative | Categorical  |  group-wise means       | [Little_app_T](`r T_app$url`)
Categorical | Quantitative |  linear regression      | [Little_App_Regression](`r Regression_app$url`)
Categorical | Categorical |  linear regression      | [Little_App_Regression](`r Regression_app$url`)
...
1. View the data with Regression, using the [Little_app_Regression](`r Regression_app$url`). This will let you readily see which type each variable is. (The choice of one variable as response and one as explanatory isn't critical here, since the reverse choice would generate the same plot but turned around the diagonal to reverse the axes.)

2. Use the Regression Little App to visualize  the relationship. Characterize it as "strong",  "moderate", "weak" or  "none". You don't need to give a more detailed description. We just want you to decide whether you can see a clear relationship or not.
...
5. Say whether the relationship (if any) shown by the statistics in the Little App is clearer than the relationship you discerned in step (2) from the point plot. 
...
* Source Package: `Little Apps`, Data set: `Natality_2014`
...
* Source Package: `Little Apps`, Data set`NHANES2`
    - Systolic blood pressure and sex.
    - Diabetes and age
    - Weight and body mass index (BMI)
    - Income and `home_type`
    
* Source Package: `mosaic`, Data set`CPS85`
    - The age of a person and wage of the person.
    - The sex of a person and wage of the person.
